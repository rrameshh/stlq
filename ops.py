...
class LogQuantize(QuantizedOperator):
    def __init__(self, momentum=0.1, device=None):
        super().__init__(momentum, device)
        self.register_buffer('scale', torch.tensor(1.0, device=device))
        
    def log_quantize(self, x):
        with torch.no_grad():
            abs_x = torch.abs(x)
            
            q = torch.zeros_like(x)
            non_zero_mask = abs_x > 1e-8
            q[non_zero_mask] = torch.sign(x[non_zero_mask]) * torch.floor(
                torch.log2(abs_x[non_zero_mask] / self.scale) + 1e-8)
            q = torch.clamp(q, -127, 127).round().to(torch.int8)
        return q
        
    def forward(self, input: torch.Tensor) -> QuantizedTensor:
        if self.activation_quantization:
            self.update_min_max_stats(input)
            self.scale = torch.max(torch.abs(input))
            q = self.log_quantize(input)
            return QuantizedTensor(q, self.scale, torch.zeros_like(self.scale).to(torch.int8))
        else:
            self.update_min_max_stats(input)
            return input


class STLQQuantize(QuantizedOperator):
    def __init__(self, ratio=0.1, momentum=0.1, device=None):
        super().__init__(momentum, device)
        self.ratio = ratio
        self.register_buffer('selection_mask', None)
        self.register_buffer('scale', torch.tensor(1.0, device=device))
    ...
    def forward(self, input: torch.Tensor) -> QuantizedTensor:
        if self.activation_quantization:
            ...
            # First round quantization
            q1 = self.log_quantize(input)
            r1 = input - q1 * self.scale
            
            # Determine selection mask if not set
            if self.selection_mask is None:
                residual_magnitude = torch.abs(r1)
                k = int(self.ratio * input.numel())
                threshold = torch.topk(residual_magnitude.flatten(), k, largest=True).values[-1]
                self.selection_mask = (residual_magnitude >= threshold).float()
            
            # Second round quantization for selected elements
            q2 = self.log_quantize(r1)
            r2 = r1 - q2 * self.scale
            
            # Combine results
            q_final = q1 + self.selection_mask * q2
            final_output = q_final * self.scale
            
            if self.training:
                return QuantizedTensor(q_final, self.scale, torch.zeros_like(self.scale).to(torch.int8), 
                                     input + (final_output - input).detach())
            else:
                return QuantizedTensor(q_final, self.scale, torch.zeros_like(self.scale).to(torch.int8))
        else:
            self.update_min_max_stats(input)
