...
def _safe_torch_load(file_path, map_location=None):
    """
    Safe torch.load that handles weights_only parameter compatibility
    """
    try:
        # Try with weights_only=False for newer PyTorch versions
        return torch.load(file_path, map_location=map_location, weights_only=False)
    except TypeError:
        # Fall back to old behavior if weights_only parameter doesn't exist
...
def load_model(load_path: str, device: str = 'auto', prefer_quantized: bool = True):
    ...
    if '_quantized.pth' in str(load_path):
        ...
        if prefer_quantized and quantized_path.exists():
            ...
            # Try to load as regular PyTorch model
            print(f"Loading as regular PyTorch model: {load_path.name}")
            return _safe_torch_load(load_path, map_location=device)
        else:
            raise FileNotFoundError(f"No model found at {load_path}")
...
def load_quantized_model(load_path: str, device: str = 'auto'):
    ...
    # Load data
    save_data = _safe_torch_load(load_path, map_location=device)
...
def get_model_info(load_path: str) -> Dict[str, Any]:
    """Get model information without loading the full model"""
    save_data = _safe_torch_load(load_path, map_location='cpu')
