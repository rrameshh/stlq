# networks/unified_mobilenetv3.py
import torch
...
def mobilenetv3_large_factory(quantization_method="linear", **kwargs):
    """
    Create MobileNetV3-Large with any quantization method.
    
    Args:
        quantization_method: "linear" or "log"
        **kwargs: device, threshold, num_classes, etc.
    """
    return mobilenetv3_large(quantization_method=quantization_method, **kwargs)

def mobilenetv3_small_factory(quantization_method="linear", **kwargs):
    """
    Create MobileNetV3-Small with any quantization method.
    
    Args:
        quantization_method: "linear" or "log"  
        **kwargs: device, threshold, num_classes, etc.
    """
    return mobilenetv3_small(quantization_method=quantization_method, **kwargs)